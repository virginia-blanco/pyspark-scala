...
spark = SparkSession.builder.appName("VBR-minio-read-and-process-"+decade).config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()

# Read the datatype into a DataFrame
df = spark.read.option("mode", "DROPMALFORMED").csv('s3a://itmd521/'+decade+'.txt')
...
splitDF.write.mode("overwrite").parquet("s3a://vblancoravena/"+decade+"-parquet")
# splitDF.write.mode("overwrite").json("s3a://vblancoravena/30-json")
splitDF.write.mode("overwrite").csv("s3a://vblancoravena/"+decade+"-csv")
# splitDF.write.mode("overwrite").option("compression", "lz4").csv("s3a://vblancoravena/"+decade+"-lz4")
# splitDF.repartition(1).write.mode("overwrite").csv("s3a://vblancoravena/"+decade+"-single-part-csv")
