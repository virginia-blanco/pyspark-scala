...
spark = SparkSession.builder.appName("VBR read minio test").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.parquet("s3a://vblancoravena/30-parquet")
splitDF.write.csv("s3a://vblancoravena/30-csv")
