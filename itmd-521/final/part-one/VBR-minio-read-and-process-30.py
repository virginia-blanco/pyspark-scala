...
spark = SparkSession.builder.appName("VBR-minio-read-and-process-30").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.mode("overwrite").parquet("s3a://vblancoravena/30-parquet")
splitDF.write.mode("overwrite").json("s3a://vblancoravena/30-json")
splitDF.write.mode("overwrite").csv("s3a://vblancoravena/30-csv")
splitDF.write.mode("overwrite").option("compression", "lz4").csv("s3a://vblancoravena/30-lz4")
splitDF.repartition(1).write.mode("overwrite").csv("s3a://vblancoravena/30-single-part-csv")
